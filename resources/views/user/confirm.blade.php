<x-layout>
  <x-reserve-back h1Title="新規予約内容のご確認" h2Title="予約内容"
    message="下記の内容にて登録します。宜しければ登録ボタンをクリックしてください">
    <form action="{{ route('user.reserve.send') }}" method="post">
      @csrf
      ...
      <div id="btn-flex">
        <a href="{{ route('user.reserve.create', ['search_date' => session('reservation.search_date')]) }}">入力内容を訂正する</a>
