<x-layout>
  <x-reserve-back h1Title="ご予約内容の変更" h2Title="" message="">
    <div id="reserve-input">
      <p class="area-title">予約入力エリア</p>
      <form action="{{ route('user.mypage.store', ['id' => $id]) }}" method="post">
        @csrf
        <div id="input-area">
          <div id="range">
            ...
            <select name="driving_range_id">
              ...
              @foreach ($ranges as $range)
                <option value="{{ $range->id }}" 
                  @if ((int)old('driving_range_id', session('reservation.driving_range_id', $reserved->driving_range_id)) === $range->id) selected @endif >
                  {{ $range->name }}
                </option>
              @endforeach
            </select>
          ...
          <div id="number">
            ...
            <select name="number">
              <option value="">選択してください</option>
              <option value="1" @if((int)old('number', session('reservation.number', $reserved->number)) === 1) selected @endif>1名様</option>
              <option value="2" @if((int)old('number', session('reservation.number', $reserved->number)) === 2) selected @endif>2名様</option>
            </select>
            ...
            <select name="reserve_date">
              ...
              @for ($i = 0; $i < \Constant::MAX_RESERVE_DATE; $i++)
                <option value="{{ $refDate->copy()->addDay($i)->format('Y-m-d') }}"
                  @if (old('reserve_date', session('reservation.reserve_date', $reserved->reserve_date)) === $refDate->copy()->addDay($i)->format('Y-m-d')) selected @endif >
              ...
              <div>
                ...
                <select name="start_time" id="start-time">
                  ...
                  @for ($time = $openTime->copy(); $time < $closeTime; $time->addminutes(30))
                    <option value="{{ $time->format('H:i:s') }}" @if(old('start_time', session('reservation.start_time', $reserved->start_time)) === $time->format('H:i:s')) selected @endif>
                      {{ $time->format('H:i') }}
                    </option>
                  @endfor
                </select>
              ...
              <div>
                ...
                <select name="end_time" id="end-time">
                  ...
                  @for ($time = $openTime->copy()->addminutes(30); $time <= $closeTime; $time->addminutes(30))
                    <option value="{{ $time->format('H:i:s') }}" @if(old('end_time', session('reservation.end_time', $reserved->end_time)) === $time->format('H:i:s')) selected @endif>
                      {{ $time->format('H:i') }}
                    </option>
                  @endfor
                </select>
          ...
          <div id="option">
            <p class="select-title">レンタルオプション</p>
              <input type="checkbox" name="rental" class="check-box" value="1"
              @if(old('rental', session('reservation.rental', $reserved->reserveRental->id ?? null))) checked @endif id="rental"/>
                  ...
                  <option value="">選択してください</option>
                    @foreach($rentals as $rental): ?>
                      <option data-model="{{ $rental->model }}" class="club" value="{{ $rental->id }}"
                        @if ((int)old('rental_id', session('reservation.rental_id', $reserved->reserveRental->rental_id ?? null)) === $rental->id) selected @endif>
                        {{ $rental->brand }}
                      </option>
                    @endforeach
                </select>
          ...
          <div id="shower-room">
            ...
            <div>
              <input type="checkbox" name="shower" class="check-box" value="1"
               @if (old('shower', session('reservation.shower', $reserved->reserveShower->id ?? null))) checked @endif id="shower"/>
            ...
            <div>
              ...
              <select name="shower_time" id="shower-time">
                <option value="">選択してください</option>
                @for ($time = $openTime->copy(); $time < $closeTime; $time->addMinutes(30))
                  <option value="{{ $time->format('H:i:s') }}"  @if(old('shower_time',
                   session('reservation.shower_time', $reserved->reserveShower->shower_time ?? null)) === $time->format('H:i:s')) selected @endif>
    ...
    <div id="reserve-info">
      ...
      <div id="information-area">
        <div class="date-weather-flex">
          <x-search-date :refDate="$refDate" :searchDate="$searchDate" :id="$id"/>
